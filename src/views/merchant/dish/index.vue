        ...
        <el-table :data="dishFlavorList" :row-class-name="rowDishFlavorIndex" @selection-change="handleDishFlavorSelectionChange" ref="dishFlavor">
          ...
          <el-table-column label="口味名称" prop="name" width="150">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.name" placeholder="请选择口味名称" @change="changeDishFlavorList(scope.row)">
                <el-option
          ...
          <el-table-column label="口味列表" prop="value" width="150">
            <template #default="scope">
              ...
              <el-select v-model="scope.row.value" placeholder="请选择口味列表" multiple @focus="dishFlavorListFocus(scope.row)">
                <el-option
...
//定义改变口味名称时更新当前口味列表
function changeDishFlavorList(row) {
  // 根据row.name获取对应的value
  const value = dishFlavorListSelect.value.find(item => item.name === row.name).value;
  row.value = [];
  // 更新当前选中口味列表
  checkedDishFlavorList.value = value;
}

//定义口味列表获取焦点时更新当前选中的口味列表
function dishFlavorListFocus(row) {
  // 根据row.name获取对应的value
  const value = dishFlavorListSelect.value.find(item => item.name === row.name).value;
  // 更新当前选中口味列表
  checkedDishFlavorList.value = value;
  console.log(dishFlavorListSelect.value.find(item => item.name === row.name).value);
  console.log(dishFlavorListSelect.value.find(item => item.name === row.name).name);
}   
